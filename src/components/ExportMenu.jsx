...
export default function ExportMenu({ onPrint, onExportPNG, onExportPDF, isFloating }) {
  ...
  return (
    <div className="relative no-print" ref={menuRef}>
      {isFloating ? (
        /* Sleek Circular Floating Trigger Icon Button */
        <button
        ...
        >
          {isOpen ? (
            <Icons.ChevronDown className="h-5 w-5 rotate-180 transition-transform duration-200" />
          ) : (
            <Icons.Download className="h-5 w-5" />
          )}
        </button>
      ) : (
        /* Standard Wide Button (Used in the baseline footer card) */
        <button
          onClick={() => setIsOpen(!isOpen)}
